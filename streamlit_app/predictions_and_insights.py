...
def iterative_predictions(model, sales_df, input_date, store='All', family='All'):
    ...
    input_date = datetime.strptime(input_date, "%Y-%m-%d")  # Assuming input_date is a string
    ...
    while last_date < input_date:
        ...
        df_temporary = df_temporary[df_temporary['date'] >= last_date + timedelta(days=-15)]
        ...
        if store == 'All' and family == 'All':
            store_family_combinations = [(store, family) for store in sales_df['store_nbr'].unique() for family in
                                         sales_df['family'].unique()]
        elif store == 'All':
            store_family_combinations = [(store, family) for store in sales_df['store_nbr'].unique()]
        elif family == 'All':
            store_family_combinations = [(store, family) for family in sales_df['family'].unique()]
        else:
            store_family_combinations = [(store, family)]

        for store, family in store_family_combinations:
            new_rows.append({'date': last_date, 'store_nbr': store, 'family': family, 'sales': 0,
                             'onpromotion': 0})  # Assume 'onpromotion' is a required feature; adjust as necessary

        df_new_rows = pd.DataFrame(new_rows)
        df_temporary = pd.concat([df_temporary, df_new_rows], ignore_index=True)

        df_temporary = prepare_data(df_sales=df_temporary, df_stores=stores_df, df_holidays=holidays_df)
        ...
        for _, row in df_new_rows.iterrows():
            ...
            input_ready = input_ready.drop(['date', 'sales','family'], axis=1)  # Drop non-feature columns; adjust as needed
            predicted_sales = model.predict(input_ready)
            # Update the original sales_df with the predicted sales
            sales_df = sales_df.append({'date': last_date, 'store_nbr': row['store_nbr'], 'family': row['family'],
                                        'sales': predicted_sales[0]}, ignore_index=True)

    # Return the updated sales_df with predictions
    return sales_df

def show_predictions_and_insights(store_nbr='All', product_family='All', apply_changes_btn=False):
    ...
    prediction_date = st.date_input("Enter prediction end date",
    ...
    if apply_changes_btn or True:  # Check if "Apply Changes" is clicked
        ...
        predicted_sales_df = iterative_predictions(model, sales_df, prediction_date.strftime("%Y-%m-%d"), store_nbr, product_family)

        # Filter the DataFrame for the last 15 days and the prediction period
        plot_data = predicted_sales_df[(predicted_sales_df['date'] > last_date - timedelta(days=15)) & (predicted_sales_df['date'] <= prediction_date)]
        ...
        ax.plot(plot_data[plot_data['date'] <= last_date]['date'], plot_data[plot_data['date'] <= last_date]['sales'], label='Actual Sales', color='blue')
        ax.plot(plot_data[plot_data['date'] > last_date]['date'], plot_data[plot_data['date'] > last_date]['sales'], label='Predicted Sales', color='red', linestyle='--')
